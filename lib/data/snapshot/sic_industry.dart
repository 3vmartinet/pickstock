...
/// `description` field on its submissions endpoint is empty for every
/// operating filer, and Item 1 "Business" exists only as prose inside the
/// 10-K document. The industry title behind a filer's SIC code is the closest
/// thing EDGAR states in a field, and is what its own `sicDescription`
/// returns — "Electronic Computers" for Apple, "Retail-Variety Stores" for
/// Walmart.
///
/// Held here rather than fetched, because the SIC code is already ingested
/// with everything else: reading the title from a table costs no request and
/// works with the network off, which is how the rest of a report is built.
/// The titles are SEC's own wording from its published SIC code list,
/// verbatim down to the spelling in code 2836.
abstract final class SicIndustry {
